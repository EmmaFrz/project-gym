...
<?php
	...
	$datos=$con->query("SELECT * FROM administrador WHERE id = '{$_GET['id']}'");
	$data = mysqli_fetch_assoc($datos);
	$datab=$con->query("SELECT * FROM permisos");
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content container-fluid">
			<form action="modi_encargado.php" method="POST">
				<input type="hidden" name="id2" value="<?php echo $_GET['id2'] ?>">
				<input type="hidden" name="id" value="<?php echo $data['id']; ?>">
				<div class="box-body">
						<div class="form-group">
							<label>Nombre</label>
							<input type="text" class="form-control" pattern="[A-Z a-z]+" name="nombre" value="<?php echo $data['nombre']; ?>" required>
						</div>
							<label>Email</label>
							<input type="email" class="form-control"name="correo" value="<?php  echo $data['correo']; ?>" required>
						</div>
						<div class="form-group">
